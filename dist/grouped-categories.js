...
const adjustTickLabelOverflow = (axis, groupedTick, leaves, depth) => {
...
const tickProto = Tick.prototype;
...
axisProto.render = function () {
    ...
    const visible = axis.hasVisibleSeries || axis.hasData(); // #185
...
axisProto.setCategories = function (newCategories, doRedraw) {
    ...
    if (axis.categories.every((cat) => isString(cat))) {
        protoAxisSetCategories.call(axis, axis.categories, doRedraw);
    }
};
axisProto.cleanGroups = function () {
    ...
    walk((axis.categoriesTree || []), 'categories', (group) => {
        ...
        tick.label?.destroy();
        objectEach(tick, (_v, i) => delete tick[i]);
        delete group.tick;
        return true;
    });
...
tickProto.addLabel = function () {
    ...
    if (tick.label) {
        ...
        tick.label.attr('text', formatter({
            axis: axis, // TODO: fix it
            tick: tick, // TODO: fix it
            chart: axis.chart,
...
tickProto.addGroupedLabels = function (category) {
    ...
    const attr = { align: 'center', rotation: options.rotation, x: 0, y: 0, style: undefined };
    ...
    while (currentTick) {
        ...
        if (currentCategory) {
            currentTick = currentTick.parent = currentCategory.tick || {};
        }
        else {
