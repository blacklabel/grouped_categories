  ...
  tickProto.addGroupedLabels = function (category) {
    ...
    while (tick) {
      if (depth > 0 && !category.tick) {
        ...
        var treeCat = axis.categories && axis.categories[this.pos];
        var fix = treeCat ? fixOffset(treeCat) : 0;

        var minPos = tickPosition(this, mathMax(this.pos - 1, axis.min - 1));
        var maxPos = tickPosition(this, mathMin(this.pos + category.leaves - 1 - fix, axis.max));

        // css should only be set for non styledMode configuration. #167
        if (label && !chart.styledMode) {
          label.css(merge(mergedCSS, {width: maxPos.x - minPos.x - 20 + 'px'}));
        }
